...
class EtatRealisationProvider extends ChangeNotifier {
    ...
    try {
      ...
      if (result.isNotEmpty && result[0].rawAddress.isNotEmpty) {
        ...
        Uri getAllComments = Uri(
            scheme: "http",
            host: "195.15.228.250",
            path: "amscommentaire/import_app/commentaires/");
        ...
        if (responsecom.statusCode == 200) {
          ...
          (json.decode(responsecom.body) as List).map((usersJson) async {
    ...
    edls.forEach((usersJson) async {
      if (usersJson["valide"] == null) {
        ...
        _etat.add(EtatRealisation(
            ...
            etat:
                usersJson['avancement'] == null ? "" : usersJson['avancement'],
  ...
  Future removeEdlFromMobile(idEdl) async {
    var edlBox = await Hive.openBox<dynamic>("edl");
    int i = edlBox.values.length;
    var edl1;
    int index = 0;
    ...
    edl["logement"]['type_log']['compteur'].forEach((key, value) async {
      ...
      for (i = 1; i <= 3; i++) {
        if (checkImageExist((value['image' + i.toString()].toString()))) {
          deleteFile(value['image' + i.toString()].toString());
        }
      }
    });
    ...
    edl["logement"]['type_log']['piece'].forEach((key, value) {
      value['rubriq'].forEach((key1, value12) async {
        int i = 1;
        for (i = 1; i <= 3; i++) {
          if (checkImageExist(value12['image' + i.toString()].toString())) {
            deleteFile(value12['image' + i.toString()].toString());
          }
        }
      });
    });
    edlBox.deleteAt(index);
  }

  Future validateEdl(idEdl, pdfPath) async {
    dynamic edl = await getSingleEdl(idEdl.toString());
    Uri editEdlRoad = Uri(
        scheme: "http",
        host: "195.15.228.250",
        path: "edlplanning/edl/updatefinal");
    //creation de la requette
    var request = http.MultipartRequest(
        'PUT', Uri.parse("http://195.15.228.250/edlplanning/edl/updatefinal"));
      ...
      for (i = 1; i <= 3; i++) {
        if (checkImageExist(value['image' + i.toString()].toString())) {
          var file = await http.MultipartFile.fromPath(
              key.toString() + '_image' + i.toString(),
              value['image' + i.toString()].toString(),
              contentType: MediaType('application', 'octet-stream'),
              filename: key.toString() + '_image' + i.toString() + ".jpg");
        }
      }
    });
    //ajout des images cles
    ...
    edl["logement"]['type_log']['piece'].forEach((key, value) {
      value['rubriq'].forEach((key1, value12) async {
        ...
        for (i = 1; i <= 3; i++) {
          if (checkImageExist(value12['image' + i.toString()].toString())) {
            var file = await http.MultipartFile.fromPath(
                key1.toString() + '_image' + i.toString(),
                value12['image' + i.toString()].toString(),
                contentType: MediaType('application', 'octet-stream'),
                filename: key1.toString() + '_image' + i.toString() + ".jpg");
            request.files.add(file);
          }
        }
      });
    ...
    var file = await http.MultipartFile.fromPath("pdf", pdfPath,
        contentType: MediaType('file', 'pdf'),
        filename: edl["_id"].toString() + ".pdf");
    request.files.add(file);
  ...
  Future getParticipantsClient(idClient, idEdl) async {
    var participantBox = await Hive.openBox<dynamic>("participant");
    var edlBox = await Hive.openBox<dynamic>("edl");
    dynamic edl =
        edlBox.values.toList().where((edl) => edl["_id"] == idEdl).toList()[0];
  ...
  getSignatairesEdl(_id) async {
    ...
    List liste =
        edlBox.values.toList().where((edl) => edl["_id"] == _id).toList();
    dynamic client = liste[0]["logement"]['client'];
    client['id'] = liste[0]["logement"]['client']['_id'];
    _signataires.add(client);
    if (liste.isEmpty == false) {
      var edl = liste[0];
  ...
  checkTypeEdl(_id) async {
    var edlBox = await Hive.openBox<dynamic>("edl");
    var edl = edlBox.values.firstWhere((object) {
      return object['_id'] == _id;
    });
    if (edl['type_edl'] == "sortant") {
      return "sortant";
    } else {
      return "entrant";
    }
  ...
  void addSignatureSignatairesEdl(
      ...
      {dynamic points = null, client = null}) async {
    var edlBox = await Hive.openBox<dynamic>("edl");
    int i = edlBox.values.length;
    var edl1;
    int index = 0;
    ...
    if (client != null) {
      edl["logement"]['client']['signature'] = path;
      edl["logement"]['client']['paraphe'] = paraphe;
      if (points != null) {
        edl["logement"]['client']['point'] = points;
      } else {
        edl["logement"]['client']['point'] = null;
      }
    }
    await edlBox.putAt(index, edl);
  }

  void eraseSignatureSignatairesEdl(_id, idSigantaire, {client = null}) async {
    var edlBox = await Hive.openBox<dynamic>("edl");
    int i = edlBox.values.length;
    var edl1;
    int index = 0;
    ...
    if (client != null) {
      edl["logement"]['client']['paraphe'] = null;
    }
    await edlBox.putAt(index, edl);
  }
}
